<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            href="{{ route('courts.create') }}">Nueva pista</a>
        </h2>
    </x-slot>
                ...
                aria-labelledby="Tabla de miembros">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">Deporte</th>
                                <th scope="col" class="px-6 py-3">Activa desde</th>
                        ...
                        hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">
                                <a href="{{ route('courts.edit', $court) }}">{{ $court->name }}</a>
                            </td>
                            <td class="px-6 py-4">
                                <a href="{{ route('courts.edit', $court) }}">{{ $court->sport->name }}</a>
                            </td>
                            <td class="px-6 py-4">
                                <a href="{{ route('courts.edit', $court) }}">
                                    {{ optional($court->created_at)->format('d/m/Y') }}
                                </a>
                            </td>
                            <td class="px-6 py-4">
                            
                            <div>
                                <button id="MenuAccionesCourt" data-dropdown-toggle="AccionesCourt{{ $court->id }}"
                                ...
                                <div id="AccionesCourt{{ $court->id }}" class="z-10 hidden bg-white divide-y
                                divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                                    aria-labelledby="MenuAccionesCourt">
                                        <li>
                                            <a href="{{ route('courts.edit', $court) }}"
                                    ...
                                    <div class="py-1">
                                        <form action="{{ route('courts.destroy', $court) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
